...
import { IconShieldMedical, IconChevronLeft, IconChevronRight, IconCheckCircle, IconUpload, IconPill } from '../../../../components/Icons';
...
const STEPS = [
  ...
  { label: 'Qualification', description: 'Education details' },
  { label: 'Registration', description: 'Pharmacy council registration' },
  { label: 'Profile', description: 'Professional profile' },
...
export default function PharmacistRegisterPage() {
  ...
  const updateField = (field: keyof PharmacistFormData, value: string | boolean | File | null) => {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 3) {
      if (!formData.statePharmacyCouncil.trim()) newErrors.statePharmacyCouncil = 'State pharmacy council is required';
  ...
  const handleSubmit = async () => {
    ...
    try {
      await ApiClient.register({
        ...
        qualification: {
          primary_qualification: formData.qualification,
          university: formData.institution,
          graduation_year: formData.graduationYear,
        },
        pharmacy_registration: {
          state_pharmacy_council: formData.statePharmacyCouncil,
    ...
    return (
      <div style={{ minHeight: 'calc(100vh - 60px)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-page)' }}>
        <div style={{ textAlign: 'center', maxWidth: '480px', padding: 'var(--sp-8)' }}>
          ...
          <h2 style={{ fontSize: 'var(--text-2xl)', fontWeight: 700, color: 'var(--text-heading)', marginBottom: 'var(--sp-2)' }}>Pharmacist Registration Submitted!</h2>
          <p style={{ fontSize: 'var(--text-md)', color: 'var(--text-secondary)', marginBottom: 'var(--sp-5)', lineHeight: 1.6 }}>
            Thank you, {formData.fullName}! Your pharmacist registration has been successfully submitted for verification.
          </p>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 'var(--sp-4)', marginBottom: 'var(--sp-5)', textAlign: 'left' }}>
            ...
            <ul style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)' }}>
              <li style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--sp-2)' }}>
                ...
                Our team will review your pharmacy council registration details
              </li>
              <li style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--sp-2)' }}>
              ...
              <li style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--sp-2)' }}>
                ...
                After verification, sign in to access your pharmacist dashboard
              </li>
            </ul>
          </div>
  ...
  const renderStep1 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <input id="fullName" type="text" className="input" style={inputStyle} placeholder="e.g. Priya Sharma" value={formData.fullName} onChange={e => updateField('fullName', e.target.value)} />
      ...
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <input id="email" type="email" className="input" style={inputStyle} placeholder="you@pharmacy.com" value={formData.email} onChange={e => updateField('email', e.target.value)} />
  ...
  const renderStep2 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <select id="qual" className="input" style={inputStyle} value={formData.qualification} onChange={e => updateField('qualification', e.target.value)}>
      ...
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <input id="institution" type="text" className="input" style={inputStyle} placeholder="e.g. Manipal College of Pharmaceutical Sciences" value={formData.institution} onChange={e => updateField('institution', e.target.value)} />
      ...
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <input id="gradYear" type="number" className="input" style={inputStyle} placeholder="e.g. 2020" min="1950" max="2030" value={formData.graduationYear} onChange={e => updateField('graduationYear', e.target.value)} />
        {errors.graduationYear && <span className="form-error">{errors.graduationYear}</span>}
      </div>
      <div className="form-group" style={{ marginBottom: 0 }}>
        <label className="form-label">Qualification Document</label>
        <label style={{
          ...
          <div>
            <div style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color: 'var(--text-primary)' }}>
              {formData.qualificationDocument ? formData.qualificationDocument.name : 'Upload qualification document'}
            </div>
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>PDF, JPG or PNG (max 5MB)</div>
          </div>
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'none' }} onChange={e => updateField('qualificationDocument', e.target.files?.[0] || null)} />
        </label>
      </div>
    </div>
  );
  ...
  const renderStep3 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
      <div className="form-group" style={{ marginBottom: 0 }}>
        <label className="form-label" htmlFor="council">State Pharmacy Council</label>
        <input id="council" type="text" className="input" style={inputStyle} placeholder="e.g. Maharashtra State Pharmacy Council" value={formData.statePharmacyCouncil} onChange={e => updateField('statePharmacyCouncil', e.target.value)} />
        {errors.statePharmacyCouncil && <span className="form-error">{errors.statePharmacyCouncil}</span>}
      </div>
      <div className="form-group" style={{ marginBottom: 0 }}>
        <label className="form-label" htmlFor="regNum">Registration Number</label>
        <input id="regNum" type="text" className="input" style={inputStyle} placeholder="e.g. MH-2020-4567" value={formData.registrationNumber} onChange={e => updateField('registrationNumber', e.target.value)} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-4)' }}>
        ...
        <div className="form-group" style={{ marginBottom: 0 }}>
          ...
          <input id="expDate" type="date" className="input" style={inputStyle} value={formData.expiryDate} onChange={e => updateField('expiryDate', e.target.value)} />
      ...
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <label style={{
          ...
          <div>
            <div style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color: 'var(--text-primary)' }}>
              {formData.certificate ? formData.certificate.name : 'Upload pharmacy council certificate'}
            </div>
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>PDF, JPG or PNG (max 5MB)</div>
          </div>
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'none' }} onChange={e => updateField('certificate', e.target.files?.[0] || null)} />
        </label>
      </div>
    </div>
  );
  ...
  const renderStep4 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <textarea id="bio" className="input textarea" placeholder="Brief description of your professional experience and expertise..." value={formData.bio} onChange={e => updateField('bio', e.target.value)} style={{ minHeight: '100px' }} />
      </div>
      <div className="form-group" style={{ marginBottom: 0 }}>
        <label className="form-label" htmlFor="specs">Specializations</label>
        <input id="specs" type="text" className="input" style={inputStyle} placeholder="e.g. Clinical Pharmacy, Oncology Pharmacy" value={formData.specializations} onChange={e => updateField('specializations', e.target.value)} />
        <span className="form-helper">Comma-separated list of your areas of expertise</span>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-5)' }}>
      <div style={{ padding: 'var(--sp-4)', background: 'var(--bg-soft)', borderRadius: 'var(--radius-md)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-2)', fontSize: 'var(--text-sm)' }}>
          ...
          <div><span style={{ color: 'var(--text-muted)' }}>Email:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.email}</span></div>
      ...
      <div style={{ padding: 'var(--sp-4)', background: 'var(--bg-soft)', borderRadius: 'var(--radius-md)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-2)', fontSize: 'var(--text-sm)' }}>
          <div><span style={{ color: 'var(--text-muted)' }}>Degree:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.qualification}</span></div>
          <div><span style={{ color: 'var(--text-muted)' }}>Institution:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.institution}</span></div>
          <div><span style={{ color: 'var(--text-muted)' }}>Year:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.graduationYear}</span></div>
        </div>
      </div>
      <div style={{ padding: 'var(--sp-4)', background: 'var(--bg-soft)', borderRadius: 'var(--radius-md)' }}>
        <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 'var(--sp-3)' }}>Pharmacy Registration</h4>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-2)', fontSize: 'var(--text-sm)' }}>
          <div><span style={{ color: 'var(--text-muted)' }}>Council:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.statePharmacyCouncil}</span></div>
          <div><span style={{ color: 'var(--text-muted)' }}>Reg No:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.registrationNumber}</span></div>
          <div><span style={{ color: 'var(--text-muted)' }}>Registered:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.registrationDate}</span></div>
          <div><span style={{ color: 'var(--text-muted)' }}>Expires:</span> <span style={{ color: 'var(--text-primary)' }}>{formData.expiryDate}</span></div>
        </div>
      </div>
      <div className="form-group" style={{ marginBottom: 0 }}>
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 60px)', background: 'var(--bg-page)' }}>
      {/* Header */}
      <div style={{
        background: 'linear-gradient(135deg, #189B6A 0%, #12805A 40%, #0F2B3C 100%)',
        padding: 'var(--sp-8) var(--sp-6)',
        position: 'relative',
        overflow: 'hidden',
      }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 30% 40%, rgba(24, 155, 106, 0.12) 0%, transparent 50%)' }} />
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-4)' }}>
            <div style={{ width: '56px', height: '56px', borderRadius: 'var(--radius-lg)', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <IconPill size={28} style={{ color: '#fff' }} />
            </div>
            <div>
              <h1 style={{ fontSize: 'var(--text-2xl)', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em' }}>Pharmacist Registration</h1>
